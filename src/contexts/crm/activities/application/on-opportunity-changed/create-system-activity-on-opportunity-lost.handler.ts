...
@EventsHandler(OpportunityLostEvent)
export class CreateSystemActivityOnOpportunityLostHandler implements IEventHandler<OpportunityLostEvent> {
  private readonly logger = new Logger(
    CreateSystemActivityOnOpportunityLostHandler.name,
  );
  ...
  async handle(event: OpportunityLostEvent): Promise<void> {
    ...
    try {
      ...
      this.logger.error(
        `Failed to create system activity for opportunity.lost: ${(err as Error).message}`,
      );
    }
  }
}
